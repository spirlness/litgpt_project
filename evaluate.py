...
# Suppress warning generated by PyTorch when creating tensors from read-only numpy arrays (memmap)
...
def evaluate(
    ...
    with torch.no_grad():
        for i in range(max_batches):
            ...
            x = x.to(dtype=torch.long).bitwise_and_(0xffff)
            y = y.to(dtype=torch.long).bitwise_and_(0xffff)

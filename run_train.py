...
def train(model_cfg_path: Path, train_cfg_path: Path, args: argparse.Namespace) -> None:
    ...
    if use_compile:
        # Check if the model is wrapped (e.g. by Fabric or DDP) and has a _forward_module attribute
        # If so, we compile the internal module to avoid issues with the wrapper.
        if hasattr(model, "_forward_module"):
            fabric.print("Compiling _forward_module of the wrapped model")
            model._forward_module = torch.compile(
